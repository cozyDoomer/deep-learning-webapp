...
def analyze(filename):
    ...
    bbox_preds, preds, scores = filter_predictions(tfm_image, class_output[0], bbox_output[0], anchors, thresh = 0.15)
    ...
    class_names = [learn.data.train_ds.classes[1:][pred.item()] for pred in preds] 
    print(f'predicted classes: {class_names}')

    img = draw_bbox_on_orig(image_path, tfm_image, [score.item() for score in scores] , bbox_preds, class_names)
    print(f'bbox on 256 size: {bbox_preds}')
