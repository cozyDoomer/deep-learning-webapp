...
class BBLossMetrics(LearnerCallback):
    ...
    def on_train_begin(self, **kwargs):
        ...
        if not self.names:
            warn('LossMetrics requested by no loss_func.metric_names provided')
    ...
    def on_batch_end(self, last_target, train, **kwargs):
        ...
        for name in self.names:
            self.metrics[name] += bs * \
                self.learn.loss_func.metrics[name].detach().cpu()
        self.nums += bs
...
class BBMetrics(LearnerCallback):
    ...
    def on_train_begin(self, **kwargs):
        ...
        if not self.names:
            warn('Metrics names requested by no metrics.metric_names provided')
        self.learn.recorder.add_metric_names(self.names)
    ...
    def on_epoch_end(self, last_metrics, **kwargs):
        ...
        metrics = [self.metrics[name] /
                   self.nums for name in self.names if name in self.learn.loss_func.metrics]
...
class PascalVOCMetric(Callback):
    ...
    def __init__(self, anchors, size, metric_names: list, detect_thresh: float = 0.3, nms_thresh: float = 0.3, images_per_batch: int = -1):
        self.ap = 'AP'
    ...
    def on_batch_end(self, last_output, last_target, **kwargs):
        ...
        for bbox_gt, class_gt, clas_pred, bbox_pred in \
            ...
            bbox_pred, scores, preds = process_output(
                clas_pred, bbox_pred, self.anchors, self.detect_thresh)
    ...
    def on_epoch_end(self, last_metrics, **kwargs):
        if self.boundingBoxes.count() > 0:
            ...
            metricsPerClass = self.evaluator.GetPascalVOCMetrics(
                self.boundingBoxes, IOUThreshold=0.3)
            self.metric = max(
                sum([mc[self.ap] for mc in metricsPerClass]) / len(metricsPerClass), 0)

            for mc in metricsPerClass:
                self.metrics['{}-{}'.format(self.ap,
                                            mc['class'])] = max(mc[self.ap], 0)

            return {'last_metrics': last_metrics + [self.metric]}
        else:
            self.metrics = dict(
                zip(self.metric_names, [0 for i in range(len(self.metric_names))]))
